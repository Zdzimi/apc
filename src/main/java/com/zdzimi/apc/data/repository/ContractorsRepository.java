...
public interface ContractorsRepository extends JpaRepository<Contractor, Long> {
  ...
  @Query("select c from Contractor c left join fetch c.code co left join fetch c.proposals where co.code = ?1 and co.creationDateTime > ?2")
  Optional<Contractor> getByTenMinuteCode(String code, LocalDateTime dateTime);
